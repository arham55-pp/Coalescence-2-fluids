...
python check_convergence.py --Lx
...
# Calculate sensitivity metrics (relative to baseline Lx=6)
print("\n" + "="*60)
...
style_axis(ax1, xlabel=r'Time $t$', ylabel=r'Bridge height $h_0$',
           title=r'Sensitivity to Domain Size: $h_0(t)$')
...
style_axis(ax2, xlabel=r'Time $t$', ylabel=r'Position of minimum $x_0$',
           title=r'Sensitivity to Domain Size: $x_0(t)$')
...
style_axis(ax3, xlabel=r'Time $t$', ylabel=r'Surfactant front position $x_f$',
           title=r'Sensitivity to Domain Size: $x_f(t)$')
...
echo "Results saved to sensitivity_Lx_plots/"
