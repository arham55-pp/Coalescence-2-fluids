...
1. Uncompensated: x(t) vs t
2. Compensated: x(t)/x_a vs t (normalized by theoretical scaling)
...
# =============================================================================
# Figure 1: Uncompensated x(t)
# =============================================================================
fig1, ax1 = plt.subplots(figsize=(5, 4))

for idx, t_arr, end_idx, label in plot_config:
    ax1.loglog(
        t_arr[1:end_idx], min_x[idx][1:end_idx],
        'o', markersize=marker_size,
        markerfacecolor=colors[idx], markeredgecolor='none',
        alpha=0.7, label=label
    )

# Reference power law: t^1.5
t_ref = t2[5:70]
ax1.loglog(t_ref, 0.03 * t_ref**1.5, 'k-', linewidth=2.5, label=r'$\sim t^{3/2}$')
...
fig1.tight_layout()
fig1.savefig(Path(__file__).parent / 'XvsT_uncompensated.pdf')
print("Saved: XvsT_uncompensated.pdf")

# =============================================================================
# Figure 2: Compensated x(t)/x_a
# =============================================================================
...
# Reference power law: t^1.5 (different coefficient for compensated)
t_ref = np.logspace(-1, 2.5, 400)
ax2.loglog(t_ref, 8.4*t_ref**1.5, 'g-', linewidth=2.5, label=r'$\sim t^{3/2}$')

ax2.set_xlabel(r'$t$')
ax2.set_ylabel(r'$x/(c_v\beta\Gamma_0\sqrt{Pe}\theta^4/(3\sqrt{\pi}))$')
ax2.legend(loc='lower right', fontsize=8)

fig2.tight_layout()
fig2.savefig(Path(__file__).parent / 'XvsT_compensated.pdf')
