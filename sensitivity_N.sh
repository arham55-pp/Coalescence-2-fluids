...
for N, color, label in zip(N_values, colors, labels):
    domain_dir = f'sensitivity_N_{N}/domain'
...
style_axis(ax1, xlabel=r'Time $t$', ylabel=r'Neck height $h_0$', title=r'Sensitivity to Mesh Resolution: $h_0(t)$')
...
style_axis(ax2, xlabel=r'Time $t$', ylabel=r'Neck position $x_0$', title=r'Sensitivity to Mesh Resolution: $x_0(t)$')
...
style_axis(ax3, xlabel=r'Time $t$', ylabel=r'Drop edge $x_e$', title=r'Sensitivity to Mesh Resolution: $x_e(t)$')
